...
/// 缓存
static mut WORK_PHASES_MANAGE: Option<Arc<RwLock<Manage>>> = None;
static mut WORK_PHASES_MANAGE_DOCUMENT: Option<Arc<RwLock<Document>>> = None;

/// 管理器
static mut WORK_PHASES_MANAGER: Option<Arc<Manager>> = None;

// 声明管理器取得函数
declare_get_manager!(PhaseSetsManager, WORK_PHASES_MANAGER);

// 实现接口
#[async_trait]
impl ManagerTrait for PhaseSetsManager {
    ...
    async fn get_manage_document(&self) -> Arc<RwLock<Document>> {
        unsafe {
            if WORK_PHASES_MANAGE_DOCUMENT.is_some() {
                ...
                WORK_PHASES_MANAGE_DOCUMENT.replace(Arc::new(RwLock::new(m_doc)));
                WORK_PHASES_MANAGE_DOCUMENT.clone().unwrap()
            }
        }
    }
