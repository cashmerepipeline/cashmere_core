...
/// 缓存
static VIEW_RULES_MANAGE: OnceLock<Arc<RwLock<Manage>>> = OnceLock::new();
static VIEW_RULES_MANAGE_DOCUMENT: OnceLock<Arc<RwLock<Document>>> = OnceLock::new();

/// 管理器
static INNER: Lazy<Arc<Box<dyn AllManagerInterface>>> =
    Lazy::new(|| Arc::new(Box::new(ViewRulesManager {})));
static VIEW_RULES_MANAGER: OnceLock<Manager> = OnceLock::new();

// 声明管理器取得函数
declare_get_manager!(ViewRulesManager, VIEW_RULES_MANAGER, INNER.clone());

declare_common_manager_interface!(
    ViewRulesManager,
    VIEW_RULES_MANAGE,
    VIEW_RULES_MANAGE_DOCUMENT,
    VIEW_RULES_MANAGE_ID
);

#[async_trait]
impl AllManagerInterface for ViewRulesManager {}
#[async_trait]
impl HardCodedInterface for ViewRulesManager {}
#[async_trait]
impl EntityInterface for ViewRulesManager {}
