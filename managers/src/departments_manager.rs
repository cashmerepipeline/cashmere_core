...
use cash_core::{ids::DEPARTMENTS_MANAGE_ID, results::*};
use database;
...
/// 缓存
static mut DEPARTMENTS_MANAGE: Option<Arc<RwLock<Manage>>> = None;
static mut DEPARTMENTS_MANAGE_DOCUMENT: Option<Arc<RwLock<Document>>> = None;

/// 管理器
static mut DEPARTMENTS_MANAGER: Option<Arc<Manager>> = None;

// 声明管理器取得函数
declare_get_manager!(WorksManager, DEPARTMENTS_MANAGER);
...
impl ManagerTrait for WorksManager {
    ...
    async fn get_manage_document(&self) -> Arc<RwLock<Document>> {
        unsafe {
            if DEPARTMENTS_MANAGE_DOCUMENT.is_some() {
                DEPARTMENTS_MANAGE_DOCUMENT.clone().unwrap()
            } else {
                ...
                DEPARTMENTS_MANAGE_DOCUMENT.replace(Arc::new(RwLock::new(m_doc)));
                DEPARTMENTS_MANAGE_DOCUMENT.clone().unwrap()
            }
        }
    }
