...
/// 缓存
static CATEGORIES_MANAGE: OnceLock<Arc<RwLock<Manage>>> = OnceLock::new();
static CATEGORIES_MANAGE_DOCUMENT: OnceLock<Arc<RwLock<Document>>> = OnceLock::new();

/// 管理器
static INNER: Lazy<Arc<Box<dyn AllManagerInterface>>> =
    Lazy::new(|| Arc::new(Box::new(CategoriesManager {})));
static CATEGORIES_MANAGER: OnceLock<Manager> = OnceLock::new();

// 声明管理器取得函数
declare_get_manager!(CategoriesManager, CATEGORIES_MANAGER, INNER.clone());

declare_common_manager_interface!(
    CategoriesManager,
    CATEGORIES_MANAGE,
    CATEGORIES_MANAGE_DOCUMENT,
    CATEGORIES_MANAGE_ID
);

#[async_trait]
impl AllManagerInterface for CategoriesManager {}
#[async_trait]
impl HardCodedInterface for CategoriesManager {}
#[async_trait]
impl EntityInterface for CategoriesManager {}
