...
/// 缓存
static ATTRIBUTES_MANAGE: OnceLock<Arc<RwLock<Manage>>> = OnceLock::new();
static ATTRIBUTES_MANAGE_DOCUMENT: OnceLock<Arc<RwLock<Document>>> = OnceLock::new();

/// 管理器
static INNER: Lazy<Arc<Box<dyn AllManagerInterface>>> =
    Lazy::new(|| Arc::new(Box::new(AttributesManager {})));
static ATTRIBUTES_MANAGER: OnceLock<Manager> = OnceLock::new();

// 声明管理器取得函数
declare_get_manager!(AttributesManager, ATTRIBUTES_MANAGER, INNER.clone());

declare_common_manager_interface!(
    AttributesManager,
    ATTRIBUTES_MANAGE,
    ATTRIBUTES_MANAGE_DOCUMENT,
    ATTRIBUTES_MANAGE_ID
);

#[async_trait]
impl AllManagerInterface for AttributesManager {}
#[async_trait]
impl HardCodedInterface for AttributesManager {}
#[async_trait]
impl EntityInterface for AttributesManager {}
