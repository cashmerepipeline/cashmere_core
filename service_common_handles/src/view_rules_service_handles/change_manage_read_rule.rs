...
use manage_define::field_ids::{
    LANGUAGES_CODES_CODE_FIELD_ID, LANGUAGES_CODES_NATIVE_FIELD_ID, VIEW_RULES_MANAGE_FIELD_ID,
};
...
#[async_trait]
pub trait HandleChangeManageReadrule {
    /// 新建管理属性
    async fn handle_change_manage_read_rule(
        ...
        let modify_doc = doc! {
            format!("{}.{}.read_rule", VIEW_RULES_MANAGE_FIELD_ID, group_id): read_rule.to_owned()
        };
        ...
        match result {
            Ok(r) => Ok(Response::new(ChangeManageReadRuleResponse {
                result: r.details(),
            })),
