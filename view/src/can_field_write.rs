...
/// 实体的可写性，可否修改实体的字段
pub async fn can_field_write(manage_id: &i32, field_id: &String, role_group: &String) -> bool {
    let view_rules = if let Some(r) = query_field_view_rules(&manage_id.to_string(), field_id, role_group).await
    {
        r
    } else {
        return false;
