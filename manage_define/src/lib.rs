...
/// 读取文件为toml_map
fn get_toml_map(toml_path: &String) -> Option<toml::map::Map<String, toml::Value>> {
    let mut toml_file =
...
// 拷贝自 utils
/// 取得管理id
fn get_id(toml_map: &toml::map::Map<String, toml::Value>) -> Option<i32> {
    let result = toml_map.get("id");
    result.map(|v| v.as_integer().unwrap() as i32)
}

/// 取得管理名
fn get_name(toml_map: &toml::map::Map<String, toml::Value>) -> Option<Document> {
 ...
 * @return 管理字段列表bson
 */
fn get_schema(toml_map: &toml::map::Map<String, toml::Value>) -> Option<Bson> {
...
pub fn generate_manage_defines(src_dirs: &Vec<&str>, target_dir: &str) {
    ...
    for dir in src_dirs.iter() {
        ...
        for entry in glob(src_dir.as_ref()).expect("读取tmol文件失败") {
