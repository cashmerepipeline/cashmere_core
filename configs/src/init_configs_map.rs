...
use crate::{get_configs_file_path, get_configs_map};

pub fn init_configs_map() -> Result<(), String> {
    let configs_path: &String = &get_configs_file_path().to_string();
    let mut config_file = std::fs::File::open(PathBuf::from(configs_path))
        .unwrap_or_else(|_| panic!("{}", t!("配置文件不存在").to_string()));
    ...
    let configs_table: toml::Table = match toml::from_str(config_str.as_str()) {
        Ok(v) => v,
        Err(e) => {
            log::error!("{}: {}", t!("配置文件解析失败"), e.to_string());
            panic!("{}: {}", t!("配置文件解析失败"), e.to_string());
