...
mod tests {
    ...
    fn it_works() {
        ...
        let _root_passwd =
            futures::executor::block_on(hash_password::hash_password(&"root".to_string())).unwrap();
        let _user_passwd =
            futures::executor::block_on(hash_password::hash_password(&"user".to_string())).unwrap();
        // println!("{}", root_passwd);
        // println!("{}", user_passwd);
